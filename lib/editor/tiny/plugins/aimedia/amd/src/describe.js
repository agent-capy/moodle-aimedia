...
export const describeSelection = async(editor) => {
    ...
    try {
        // A picture that has just been pasted, or dragged in from the desktop, is
        // not a file yet. TinyMCE holds it in memory and its src is a blob: URL,
        // which means nothing to the server; the upload into the draft area
        // happens in the background, or not until the form is saved. So the
        // upload is asked for here and waited for, which is what the editor
        // itself does on save. An image that is already a file is untouched.
        await editor.uploadImages();

        // The upload rewrites the src on the same element, but the selection can
        // move while it runs, so the image is found again rather than assumed.
        const target = image.isConnected ? image : selectedImage(editor);
        if (!target) {
            return;
        }
        ...
        const response = await fetchMany([{
            ...
            args: {
                ...
                imageurl: target.getAttribute('src'),
        ...
        if (editor.removed || !target.isConnected) {
        ...
        editor.undoManager.transact(() => {
            target.setAttribute('alt', response.text);
        });
        editor.nodeChanged();
        Notification.addNotification({
